...
Revise the following image generation prompt to optimize it for Stable Diffusion, incorporating best practices:
    ...
    2. prompt must be include A detailed close-up of a beautiful female character for a game feature graphic.
...
def enhance_prompt(original_prompt, claude_enhance_params):
    ...
    1. google play storeのカジュアルゲーム用のフィーチャーグラフィックを作成するための文章を作成してください。
    2. 可愛らしい女性のキャラクターが、こちらをむかってポーズをとっているシーンを想像してください。
    3. フィーチャーグラフィックには女性のキャラクターの腰から上を描いてください。
...
if __name__ == "__main__":
    ...
    original_prompt = "18歳の学生が、日本の学校の教室で可愛くポーズをとっている。"
